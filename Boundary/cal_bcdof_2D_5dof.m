function [bcdof, bcval] = cal_bcdof_2D_5dof(IGA,Plate)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% ===== Dofs of control points on boundaries (dofs = u_{0}, v_{0}, w_{0}, \beta_{x}, \beta_{y}) =====
switch bc_case
...
%         end
    ...
    case 2 % Fully clamped (CCCC)
        for idof = [1, 2, 3, 4, 5]  % Constraint u_{0}, v_{0}, w_{0}, \beta_{x}, \beta_{y} at perimeter
            bcdof = [bcdof ndof*(left1-1)+idof];    % Left
            bcdof = [bcdof ndof*(right1-1)+idof];   % Right
            bcdof = [bcdof ndof*(upper1-1)+idof];   % Upper
            bcdof = [bcdof ndof*(lower1-1)+idof];   % Lower
